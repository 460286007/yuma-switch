...
fn merge_settings_for_save(
    ...
    // git 账号与当前平台由专用命令（save_git_accounts / switch_git_platform）
    // 维护，普通设置保存不携带这两个字段时保持现有值。
    if incoming.git_accounts.is_none() {
        incoming.git_accounts = existing.git_accounts.clone();
    }
    if incoming.current_git_platform.is_none() {
        incoming.current_git_platform = existing.current_git_platform.clone();
    }
