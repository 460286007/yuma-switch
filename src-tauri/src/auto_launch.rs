...
/// 清理改名前（CC Switch 时代）留下的开机自启条目，避免任务管理器启动项出现两个。
pub fn cleanup_legacy_auto_launch() {
    // 旧版本曾以 "CC Switch" 名称注册自启；改名后尽量移除残留。
    for legacy_name in ["CC Switch", "cc-switch"] {
        if let Ok(legacy) = get_auto_launch_named(legacy_name) {
