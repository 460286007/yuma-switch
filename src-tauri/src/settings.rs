...
/// 单个平台的 git 账号（仅存储在本机 settings，用于切换全局 git 身份）
...
pub struct GitAccount {
    ...
    #[serde(default)]
    pub email: String,
    /// 密码不参与 git config（git 身份只需要 name/email），仅本地留存备用。
    #[serde(default)]
    pub password: String,
    /// 该账号对应的本地项目路径（选填，仅记录，不参与 git config）
    #[serde(default)]
    pub project_path: String,
}

/// 兼容旧版（每平台单账号对象）与新结构（账号数组）的反序列化
#[derive(Deserialize)]
#[serde(untagged)]
enum OneOrManyGitAccount {
    One(GitAccount),
    Many(Vec<GitAccount>),
}

fn deserialize_git_account_list<'de, D>(deserializer: D) -> Result<Vec<GitAccount>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    Ok(match OneOrManyGitAccount::deserialize(deserializer)? {
        OneOrManyGitAccount::One(account) => vec![account],
        OneOrManyGitAccount::Many(list) => list,
    })
}

/// gitee / github 两套 git 账号（每平台可存多个，主界面按平台分页管理）
#[derive(Debug, Clone, Serialize, Deserialize, Default, PartialEq)]
pub struct GitAccounts {
    #[serde(default, deserialize_with = "deserialize_git_account_list")]
    pub gitee: Vec<GitAccount>,
    #[serde(default, deserialize_with = "deserialize_git_account_list")]
    pub github: Vec<GitAccount>,
}
...
pub struct AppSettings {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub git_accounts: Option<GitAccounts>,
    /// 当前 git 平台选择（gitee / github）
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_git_platform: Option<String>,
    /// 当前生效的 git 账号 ID
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_git_account: Option<String>,
    ...
    /// 当前 Zcode 供应商 ID（本地存储，优先于数据库 is_current）
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Default for AppSettings {
    fn default() -> Self {
        Self {
            ...
            pi_config_dir: None,
            git_accounts: None,
            current_git_platform: None,
            current_git_account: None,
...
pub(crate) fn settings_store() -> &'static RwLock<AppSettings> {
