...
export function AboutSection({ isPortable }: AboutSectionProps) {
  ...
  // 安装入口的 in-flight 工具集合。
  // 覆盖 executeRun 内部 setToolActions 落到 React commit 前的几个 microtask 窗口：
  // 在它之前按钮不会 disabled → 用户快速双击会并发开两轮 `npm i -g`（写冲突）。
  // 把执行期间的工具登记在这里、纳入 isAnyBusy 派生，关掉这个并发窗口。
  ...
  const executeRun = useCallback(
    async (toolNames: ToolName[]) => {
      ...
      for (const toolName of toolNames) {
        setToolActions((prev) => ({ ...prev, [toolName]: "install" }));
        try {
      ...
      const actionLabel = t("settings.toolInstall");
  ...
  // 安装的统一入口锁：入口处先登记 preflight、出口处解锁，
  // 覆盖 executeRun 内部 setToolActions 落到 React commit 前的 microtask 窗口，
  // 避免快速双击并发触发两轮 `npm i -g`（全局写入冲突）。
  ...
  // 任一安装进行中即视为忙碌：用于禁用所有操作按钮，
  // 避免并发触发多个 npm 全局写入造成冲突。
  const isAnyBusy =
    Object.keys(toolActions).length > 0 || preflightTools.size > 0;
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-8">
            <div className="flex flex-col items-center gap-2">
              <div className="flex items-center gap-2">
                <img src={appIcon} alt="Yuma Switch" className="h-5 w-5" />
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => settingsApi.openExternal("https://www.ityuma.com/")}
                ...
                <div className="mt-auto flex items-center justify-end">
                  {isToolVersionLoading ? (
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="h-7 gap-1.5 text-xs"
                      onClick={() => handleRunToolAction([toolName])}
                      disabled={isToolVersionLoading || isAnyBusy}
                    >
                      {/* 入口锁登记期间也转圈：此时 toolActions 尚未置位，
                          只 disabled 不转圈会让数秒像"点了没反应"。 */}
